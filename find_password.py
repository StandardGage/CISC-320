...
def access_path(data: dict or any, path: list[str]) -> any:
    ...
    if path:
        temp = path[0]
        path = path[1:]
        return access_path(data[temp], path)
    return data
...
def solve(maze: str, at: int, visited: set[int], password=None) -> str:
    """
    ...
    if maze:
        ...
        match instruction:
            case '→':
                password = solve(maze, at + int(maze[at + 1]), visited)
            case '↕':
                password = solve(maze, at - int(maze[at + 1]), visited)
                if password:
                    return password
                password = solve(maze, at + int(maze[at + 2]), visited)
                return password
            case '←':
                password = solve(maze, at - int(maze[at + 1]), visited)
            case '↖':
...
if __name__ == '__main__':
    main(["SubBasement 3B", "Zone 1", "Containment 3", "West"], 15)
